...
const int numReadings = 20; // Number of readings to include in the moving average
int readings[numReadings];  // Array to store the readings
int indexB = 0;             // Index of the current reading
int total = 0;              // Running total of the readings
const int gain40 = 100;
const float gain50 = 316.2278;
const int gain60 = 1000;
const float referencePressure = 0.000020; // 20 µPa reference pressure
...
void loop()
{
    ...
    const float soundPressure = ((outputVoltage)/(sensitivity*gain60));  // change the gain if it is connected to another pin

    if (soundPressure == 0)
    {
       soundLevel = 15.6; // 60dB gain = 15.6  50dB gain = 25.3   40dB gain = 35
    } else

    soundLevel = (20 * log10(soundPressure / referencePressure) * 0.97);  // calibration factor average is 0.97 (3%)
